...
def getDiskDetailsDefault(host, port, user, password, dbname, host_name, field_name):
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT mean("+field_name+") FROM disk WHERE host ='"+host_name+"' AND time > now() - 7d GROUP BY time(1h) fill(0)"
...
def getDiskDetails(host, port, user, password, dbname, host_name,field_name,from_date,to_date):
	...
	if(group_val < 10):
		group_val = 10
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT mean("+field_name+") FROM disk  WHERE host='"+host_name+"' AND (time >= '"+from_date+"' AND time <= '"+to_date+"') GROUP BY time("+str(group_val)+"s) fill(0)"
	data = client.query(query)
	dataframe = data['disk']
	dict={}
	dict["mean"]=json.loads(dataframe['mean'].to_json(orient='values',force_ascii=True))
...
def getDiskFieldList(host, port, user, password, dbname):
	client = DataFrameClient(host, port, user, password, dbname)
	query ="show field keys from disk"
	data = client.query(query)
	dataframe = data['disk']
	field_list = []
	for x in dataframe:
...
def disk_arima(host, port, user, password, dbname, query,number_of_prediction):
...
def disk_ar(host, port, user, password, dbname, query,number_of_prediction):
...
def disk_ma(host, port, user, password, dbname, query,number_of_prediction):
...
def disk_holt(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['disk']
	dict={}
...
def disk_holtwinter(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['disk']
	dict={}
