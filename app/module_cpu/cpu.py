...
def getCpuDetailsDefault(host, port, user, password, dbname, host_name , field_name):
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT mean("+field_name+") FROM cpu  WHERE cpu = 'cpu-total' AND host='"+host_name+"' AND time > now() - 7d GROUP BY time(1h) fill(0)"
...
def getCpuFieldList(host, port, user, password, dbname):
	client = DataFrameClient(host, port, user, password, dbname)
...
def getCpuDetails(host, port, user, password, dbname, host_name,field_name,from_date,to_date):
	...
	if(group_val < 10):
		group_val = 10
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT mean("+field_name+") FROM cpu  WHERE cpu = 'cpu-total' AND host='"+host_name+"' AND (time >= '"+from_date+"' AND time <= '"+to_date+"') GROUP BY time("+str(group_val)+"s) fill(0)"
	data = client.query(query)
	dataframe = data['cpu']
	dict={}
	dict["mean"]=json.loads(dataframe['mean'].to_json(orient='values',force_ascii=True))
...
def cpu_arima(host, port, user, password, dbname, query,number_of_prediction):
...
def cpu_ar(host, port, user, password, dbname, query,number_of_prediction):
...
def cpu_ma(host, port, user, password, dbname, query,number_of_prediction):
...
def cpu_holt(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['cpu']
	dict={}
...
def cpu_holtwinter(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['cpu']
	dict={}
