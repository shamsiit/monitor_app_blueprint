...
def getKernelDetails(host, port, user, password, dbname, host_name,field_name,from_date,to_date):
	...
	if(group_val < 10):
		group_val = 10
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT mean("+field_name+")/1000000 FROM kernel  WHERE host='"+host_name+"' AND (time >= '"+from_date+"' AND time <= '"+to_date+"') GROUP BY time("+str(group_val)+"s) fill(0)"
	#print query
	data = client.query(query)
	dataframe = data['kernel']
	dict={}
...
def getKernelFieldList(host, port, user, password, dbname):
	client = DataFrameClient(host, port, user, password, dbname)
	query ="show field keys from kernel"
	data = client.query(query)
	dataframe = data['kernel']
	field_list = []
	for x in dataframe:
...
def kernel_arima(host, port, user, password, dbname, query,number_of_prediction):
...
def kernel_ar(host, port, user, password, dbname, query,number_of_prediction):
...
def kernel_ma(host, port, user, password, dbname, query,number_of_prediction):
...
def kernel_holt(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['kernel']
	dict={}
...
def kernel_holtwinter(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['kernel']
	dict={}
