...
def getDiskioDetailsDefault(host, port, user, password, dbname, host_name , field_name):
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT non_negative_derivative(mean("+field_name+"), 10s)/1048576 as mean FROM diskio WHERE host ='"+host_name+"' AND time > now() - 7d GROUP BY time(1h) fill(0)"
	data = client.query(query)
	dataframe = data['diskio']
	dict={}
...
def getDiskioDetails(host, port, user, password, dbname, host_name,field_name,from_date,to_date):
	...
	if(group_val < 10):
		group_val = 10
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT non_negative_derivative(mean("+field_name+"), 10s)/1048576 as mean FROM diskio  WHERE host='"+host_name+"' AND (time >= '"+from_date+"' AND time <= '"+to_date+"') GROUP BY time("+str(group_val)+"s) fill(0)"
	#print query
	data = client.query(query)
	dataframe = data['diskio']
	dict={}
...
def getDiskioFieldList(host, port, user, password, dbname):
	client = DataFrameClient(host, port, user, password, dbname)
	query ="show field keys from diskio"
	data = client.query(query)
	dataframe = data['diskio']
	field_list = []
	for x in dataframe:
...
def diskio_arima(host, port, user, password, dbname, query,number_of_prediction):
...
def diskio_ar(host, port, user, password, dbname, query,number_of_prediction):
...
def diskio_ma(host, port, user, password, dbname, query,number_of_prediction):
...
def diskio_holt(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['diskio']
	dict={}
...
def diskio_holtwinter(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['diskio']
	dict={}
