...
def getProcessesDetailsDefault(host, port, user, password, dbname, host_name, field_name):
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT mean("+field_name+") FROM processes WHERE host ='"+host_name+"' AND time > now() - 7d GROUP BY time(1h) fill(0)"
	data = client.query(query)
	dataframe = data['processes']
	dict={}
...
def getProcessesDetails(host, port, user, password, dbname, host_name,field_name,from_date,to_date):
	...
	if(group_val < 10):
		group_val = 10
	client = DataFrameClient(host, port, user, password, dbname)
	query="SELECT mean("+field_name+") FROM processes  WHERE host='"+host_name+"' AND (time >= '"+from_date+"' AND time <= '"+to_date+"') GROUP BY time("+str(group_val)+"s) fill(0)"
	data = client.query(query)
	dataframe = data['processes']
	dict={}
	dict["mean"]=json.loads(dataframe['mean'].to_json(orient='values',force_ascii=True))
...
def getProcessesFieldList(host, port, user, password, dbname):
	client = DataFrameClient(host, port, user, password, dbname)
	query ="show field keys from processes"
	data = client.query(query)
	dataframe = data['processes']
	field_list = []
	for x in dataframe:
...
def processes_arima(host, port, user, password, dbname, query,number_of_prediction):
...
def processes_ar(host, port, user, password, dbname, query,number_of_prediction):
...
def processes_ma(host, port, user, password, dbname, query,number_of_prediction):
...
def processes_holt(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['processes']
	dict={}
...
def processes_holtwinter(host, port, user, password, dbname , query,number_of_prediction):
	client = DataFrameClient(host, port, user, password, dbname)
	data = client.query(query)
	dataframe = data['processes']
	dict={}
